...
static int32 _bt_prefixKeyCompress(Relation rel, BTItem lowItem, BTItem highItem);
 ...
 */
static int32
_bt_prefixKeyCompress(Relation rel, BTItem lowItem, BTItem highItem)
{
  TupleDesc itupdesc;
   ...
   * which is a pointer to a C string.  Note that vl_len is the length of the
   * whole text struct, which includes 4 bytes for the vl_len field.  So the
   * length of the vl_dat is actually (vl_len - 4).
   */
  text *lowText;
  text *highText;
  int32 toReturn;
  ...
  /* CS448 STUDENTS: this errbuf variable is used to format required output messages */
  /*   DO NOT MODIFY IT                                                              */
  /* If you wish to generate additional debugging output using elog, declare         */
  /*   another buffer to hold your output                                            */  
  ...
  if(ATTR_IS_TEXT(attr)) {
    ...
    /*
     * CS448 STUDENTS: DO NOT CHANGE THE LINES BETWEEN THIS COMMENT
     *  AND THE MATCHING CLOSING COMMENT BELOW!!!
     * This code is intended to extract and print the high and low strings.
     * THIS OUTPUT WILL BE USED DURING MARKING
     */
    /*
     ...
     */
    highDatum = BTIndexTupleGetDatum(&(highItem->bti_itup));
    lowDatum = BTIndexTupleGetDatum(&(lowItem->bti_itup)); 
    ...
    pos = sprintf(errbuf, "CS448 HIGH: ");
    for (i=0; i < highText->vl_len - 4; i++) /* remember vl_dat is 4 shorter! */
      sprintf(&(errbuf[pos++]), "%c", highText->vl_dat[i]);
    pos += sprintf(&(errbuf[pos]), " LOW: ");
    for (i=0; i < lowText->vl_len - 4; i++)
      sprintf(&(errbuf[pos++]), "%c", lowText->vl_dat[i]);
    /* 
     * CS448 STUDENTS: THIS CLOSING COMMENT MARKS THE END OF THE CODE YOU SHOULD NOT CHANGE
     */
     ...
     * should be (lowText->vl_len - 4) and similar for highp (to start with).
    ...
    /*
     * CS448 STUDENTS: DO NOT CHANGE THE 5 LINES FOLLOWING THIS COMMENT.
     * THEY PRODUCE OUTPUT THAT WILL BE USED DURING MARKING
     */
    pos += sprintf(&(errbuf[pos]), " SAVED: %d COMPRESSED: ", toReturn);
    for (i=0; i < highText->vl_len - 4; i++) /* remember vl_dat is 4 shorter! */
      sprintf(&(errbuf[pos++]), "%c", highText->vl_dat[i]);
    errbuf[pos] = '\0';
    elog(DEBUG1, "%s", errbuf);
    /* CS448 STUDENTS: you are free to make changes below this comment */
	  ...
	  bool newitemonleft)
{
  ...
  /* Smallest item in right node. */
  Size	 itemsz;
  ItemId itemid;
  BTItem item;

  /* Largest item in left node. */
  Size   maxleftsz; 
  ItemId maxleftid;
  BTItem maxleft;
  ...
  maxleft = (BTItem) PageGetItem(origpage, maxleftid);
   ...
   */
  itemcpy = (BTItem)palloc(itemsz); /* always use palloc, NEVER malloc! */
 ...
 * 
 * Currently, this produces no output unless the server debug level
 * is at least DEBUG2
 */
void _btdumppage(Relation r, Buffer buf)
{
  ...
  for (offnum = FirstOffsetNumber;
	...
	itup = &(((BTItem) PageGetItem(page, iid))->bti_itup);	
	itblkno = ItemPointerGetBlockNumber(&(itup->t_tid));
	if (offnum != P_FIRSTDATAKEY((BTPageOpaque)PageGetSpecialPointer(page))) {
	  itoffno = ItemPointerGetOffsetNumber(&(itup->t_tid));
	  datum = BTIndexTupleGetDatum(itup);
	  itkey = ((text *)datum)->vl_dat;
	  for(i = 0; i < ((text *)datum)->vl_len - 4 && i < sizeof(tmpbuf) - 1; i++)
	    tmpbuf[i] = itkey[i];
	  tmpbuf[i] = '\0';
	  elog(DEBUG2, "\t[%d] size %lu rid <%d,%d> key:%s\n",
	       offnum, IndexTupleSize(itup), itblkno, itoffno, tmpbuf);
	}
	else elog(DEBUG2, "\t[%d] size %lu heap <%d,%d> (no key)\n",
